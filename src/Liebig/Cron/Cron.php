...
class Cron {
    ...
    public static function run($checkRundateOnce = true) {
        ...
        try {
            ...
            event('cron.beforeRun', array($runDate->getTimestamp()));
            ...
            if (is_bool($preventOverlapping) && $preventOverlapping) {
                ...
                if (!empty($storagePath)) {
                    ...
                    if (file_exists($lockFile)) {
                        ...
                        event('cron.locked', array('lockfile' => $lockFile));

                        // Fire the after run event, because we are done here
                        event('cron.afterRun', array('rundate' => $runDate->getTimestamp(), 'inTime' => -1, 'runtime' => -1, 'errors' => 0, 'crons' => array(), 'lastRun' => array()));
            ...
            foreach (self::$cronJobs as $job) {
                ...
                if ($job['enabled'] && $job['expression']->isDue($checkTime)) {
                    ...
                    if (!is_null($return)) {
                        ...
                        event('cron.jobError', array('name' => $job['name'], 'return' => $return, 'runtime' => ($afterOne - $beforeOne), 'rundate' => $runDate->getTimestamp()));
                    } else {
                        // Fire event after executing a job successfully
                        event('cron.jobSuccess', array('name' => $job['name'], 'runtime' => ($afterOne - $beforeOne), 'rundate' => $runDate->getTimestamp()));
                    }
            ...
            event('cron.afterRun', $returnArray);
